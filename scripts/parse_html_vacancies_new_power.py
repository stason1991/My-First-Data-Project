...
# Интегрирован импорт BANK_TIERS из конфигурационного файла
try:
...
def parse_html_experience(text):
    ...
    if any(x in text for x in ["нет опыта", "без опыта"]):
        return 0
    if "1–3 года" in text or "1 года" in text or "1-3 года" in text or "1–3" in text:
        return 24
    if "3–6 лет" in text or "3 лет" in text or "3-6 лет" in text or "3–6" in text:
        return 54
...
def determine_region_tier_from_html(html_content, title_text):
    ...
    city_match = (
        re.search(r'"areaName"\s*:\s*"([^"]+)"', html_content)
        or re.search(r'"cityName"\s*:\s*"([^"]+)"', html_content)
        or re.search(r'area:\s*\{\s*name:\s*"([^"]+)"', html_content)
        or re.search(r'"area"\s*:\s*\{\s*"name"\s*:\s*"([^"]+)"', html_content)
    )
...
if __name__ == "__main__":
    ...
    for file_path in tqdm(sorted(html_files), desc="Семантическая разметка матрицы X"):
        try:
            ...
            salary_el = soup.find(attrs={"data-qa": "vacancy-salary"}) or soup.find(
                class_=re.compile("salary|compensation")
            )
            ...
            is_english = (
                1
                if re.search(r"\benglish\b|английск|upper\s*intermediate|advanced|b2|c1|fluent", clean_description)
                else 0
            )
            ...
            is_grade_management = (
                1
                if re.search(
                    r"руководитель|директор|заместитель|начальник|заведующий|тимлид|team\s*lead|lead|управляющий",
                    title_lower,
                )
                else 0
            )
            is_grade_junior = (
                1
                if (is_grade_management == 0)
                and re.search(r"начинающий|стажер|стажёр|junior|джуниор|начальный.*уровень|без\s*опыта", title_lower)
                else 0
            )
            is_grade_senior = (
                1
                if (is_grade_management == 0)
                and re.search(r"главный|ведущий|senior|сеньор|сеньёр|эксперт|руководитель.*направления", title_lower)
                else 0
            )
            ...
            edu_level = 3
            if "среднее профессиональное" in html_content or "колледж" in html_content or "техникум" in html_content:
                edu_level = 1
            elif "неполное высшее" in html_content:
                edu_level = 2
            elif "высшее" in html_content:
                edu_level = 3
            elif "магистратура" in html_content or "кандидат" in html_content:
                edu_level = 4
            elif "среднее образование" in html_content:
                edu_level = 0
            ...
            row = {
                ...
                "is_grade_junior": is_grade_junior,
                "is_grade_middle": is_grade_middle,
                "is_grade_senior": is_grade_senior,
                "is_grade_management": is_grade_management,
                "edu_level_encoded": edu_level,
                "salary_from": s_from,
                "salary_to": s_to,
                "y_offer": y_offer,
                "is_vacancy": 1,
