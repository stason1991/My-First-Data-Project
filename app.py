...
st.title("Интеллектуальная система оценки кандидатов и калькуляции офферов")
st.caption("Микросервисный контур: FastAPI + Celery + PostgreSQL | Модель: XGBoost (MAPE 19.7%)")

BACKEND_URL = "http://backend:8000/api/v1"

# 17 Репрезентативных классов должностей банковской сферы из EDA
ROLES = {
    1: "Мобильный фронт / Доставка карт", 2: "Прямые продажи / DSA",
    6: "Сервисный бэк-офис", 7: "Линейные кассиры",
    8: "Менеджер по продажам (Масс-Фронт)", 9: "Главный менеджер продаж (Senior)",
    11: "Персональный менеджер VIP / Private", 13: "Менеджер по обслуживанию ЮЛ (Farmer)", 
    14: "Руководитель направления ЮЛ", 15: "Ипотечный брокер",
    17: "Кадровый резерв (Массовый)", 18: "Кадровый резерв (Старший)", 
    20: "Начальник дополнительного офиса", 21: "Data Scientist / ML Engineer", 
    24: "Системный аналитик ИТ", 25: "DevOps Engineer", 
    30: "Методолог ИБ", 31: "HR-ресечер массового подбора"
}

col1, col2 = st.columns(2)

with col1:
    st.subheader("Паспорт кандидата и Текст резюме")
    ...
    bank_tier = st.selectbox("Тир текущей финансовой организации:", ["Bank_Tier_1", "Bank_Tier_2"])
...
with col2:
    ...
    tab_soft, tab_hard, tab_biz, tab_infra = st.tabs([
        "🗣️ Soft Skills", "💻 Hard Skills", "📊 Business Skills", "🌐 Среда и Образование"
    ])
    
    with tab_soft:
        st.caption("Управление коммуникациями, стейкхолдерами и конфликтология")
        ...
        s_public = st.checkbox("Публичные выступления / Презентация решений", key="s_public")
        s_correspondence = st.checkbox("Грамотная деловая переписка (КП / E-mail)", key="s_correspondence")
        s_cold = st.checkbox("Активное привлечение / Холодные продажи", key="s_cold")
        s_tele = st.checkbox("Телемаркетинг / Телефонные продажи / Обзвоны", key="s_tele")
        s_objection = st.checkbox("Отработка негатива и работа с возражениями", key="s_objection")
        s_cross = st.checkbox("Кросс-продажи и расширение чека (Up-sell)", key="s_cross")
        s_diff_cli = st.checkbox("Управление капризами / Работа со сложными клиентами", key="s_diff_cli")
        s_conflict = st.checkbox("Разрешение споров и гашение конфликтов", key="s_conflict")
        s_stress = st.checkbox("Стрессоустойчивость к высоким нагрузкам", key="s_stress")
        s_adapt = st.checkbox("Адаптивность / Быстрая обучаемость / Agile-методологии", key="s_adapt")
        s_team = st.checkbox("Работа в команде / Кросс-функциональное взаимодействие", key="s_team")
        s_res_def = st.checkbox("Защита результатов и обоснование KPI", key="s_res_def")
        s_mentor = st.checkbox("Наставничество / Онбординг / Менторинг стажеров", key="s_mentor")
        s_proactive = st.checkbox("Высокая проактивность и ownership", value=True, key="s_proactive")
    ...
    with tab_biz:
        st.caption("Управление эффективностью, метрики, комплаенс и регуляторика")
        ...
        b_res_def_data = st.checkbox("Защита бюджетов на основе данных (Data-driven)", key="b_res_def_data")
        b_roleplay = st.checkbox("Успешное прохождение Ролевых игр / Аудита комплаенса", key="b_roleplay")
        b_field_adapt = st.checkbox("Динамическая адаптация к законам и фокусам рынка", key="b_field_adapt")
        b_learning = st.checkbox("Высокая скорость прохождения внутренних аттестаций", key="b_learning")
        b_pnl = st.checkbox("Управление P&L / Бюджетирование / Unit-экономика / Метрики", key="b_pnl")
        b_basel = st.checkbox("Банковская регуляторика рисков (Базель / Нормативы ЦБ)", key="b_basel")
        b_compliance = st.checkbox("Комплаенс-контроль (115-ФЗ / AML / KYC / Валютного контроль)", key="b_compliance")
        b_hr = st.checkbox("Управление человеческим капиталом и HR-аналитика", key="b_hr")

    with tab_infra:
        st.caption("Инфраструктурные маркеры и требования SOCIAL_INFRA_TRIGGERS")
...
if st.button("Запустить комплексный расчет оффера кандидата", type="primary"):
    ...
    payload = {
        ...
        # Передаем абсолютно все верифицированные на интервью бинарные флаги
        ...
        # Интегрируем группу SOCIAL_INFRA_TRIGGERS строго по именам колонок матрицы X
...
if "pred_id" in st.session_state:
    ...
    with st.form("feedback_form"):
        accepted = st.radio("Согласны ли вы и нанимающий менеджер с предложенной ставкой?", ["Да, оффер утвержден и отправлен кандидату", "Нет, ставка отклонена бизнесом"])
        actual_val = st.number_input("Итоговый согласованный оклад (если отличается от ИИ-рекомендации):", min_value=0, value=0)
        ...
        if st.form_submit_button("Зафиксировать решение в СУБД"):
            ...
            requests.post(f"{BACKEND_URL}/feedback", json=fb_payload)
